-- Setup trainer@fitiva.com account similar to the manager setup
...
BEGIN
    ...
    SELECT au.id INTO trainer_user_id
    FROM auth.users au 
    WHERE au.email = 'trainer@fitiva.com';
    ...
    IF trainer_user_id IS NOT NULL THEN
        RAISE NOTICE 'Found trainer user ID: %', trainer_user_id;
        ...
        IF target_org_id IS NOT NULL THEN
            ...
            VALUES (trainer_user_id, target_org_id, 'trainer');
                
            RAISE NOTICE 'Trainer account setup complete for organization: %', target_org_id;
        ELSE
            RAISE NOTICE 'Organization not found!';
        END IF;
        
    ELSE
        RAISE NOTICE 'Trainer user not found in auth.users - create the account first!';
    END IF;
END $$;
...
FROM users u 
WHERE u.email = 'trainer@fitiva.com'

UNION ALL

SELECT 'PROFILES:' as table_name, p.email, p.full_name as id
FROM profiles p 
WHERE p.email = 'trainer@fitiva.com'
